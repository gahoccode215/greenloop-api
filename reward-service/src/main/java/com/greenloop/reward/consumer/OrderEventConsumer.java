...
@Service  // ✅ Đổi từ @Component thành @Service
@Slf4j
@RequiredArgsConstructor
public class OrderEventConsumer {  // ✅ Đổi tên class

    private final EcoPointUserService ecoPointUserService;
    private final VoucherService voucherService;

    @Bean  // ✅ Thêm @Bean annotation
    public Consumer<OrderOfflineCreatedEvent> orderOfflineCreatedRewardConsumer() {
        return event -> {
            log.info("Received OrderOfflineCreatedEvent - orderId: {}, customerId: {}, earnedEcoPoints: {}, voucherUserId: {}",
                    event.getOrderId(), event.getCustomerId(), event.getEarnedEcoPoints(), event.getVoucherUserId());
    ...
    private void processVoucherRedemption(OrderOfflineCreatedEvent event) {
        ...
        try {
            ...
            log.info("Voucher redemption processed for voucherUserId: {}, orderId: {}",
                    event.getVoucherUserId(), event.getOrderId());

        } catch (Exception e) {
            log.error("Failed to process voucher redemption for orderId {}: {}",
    ...
    private void processEcoPoints(OrderOfflineCreatedEvent event) {
        ...
        try {
            ...
            log.info("Added {} eco points for customer {} - orderId: {}",
                    event.getEarnedEcoPoints(), event.getCustomerId(), event.getOrderId());

        } catch (Exception e) {
            log.error("Failed to add eco points for order {}: {}",
                    event.getOrderId(), e.getMessage(), e);
        }
    }
}
