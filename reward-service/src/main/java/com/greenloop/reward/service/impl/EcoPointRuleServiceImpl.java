...
public class EcoPointRuleServiceImpl implements EcoPointRuleService {
    ...
    @Override
    public EcoPointResponse getEcoPointInfo(EcoPointRuleRequest request) {
        String redisKey = "eco_point_rule_" + request.getActionType() + "_" + request.getCategoryId();
        EcoPointResponse cached = cacheService.get(redisKey, EcoPointResponse.class);
        if (cached != null) {
            return cached;
        }
        EcoPointRule rule =
                ecoPointRuleRepository
                        .findByActionTypeAndCategoryId(request.getActionType(), request.getCategoryId())
                        .orElseThrow(() -> new BusinessException(ErrorCode.ECO_POINT_RULE_NOT_FOUND));
        EcoPointResponse response = mapToResponse(rule);
        cacheService.store(redisKey, response);
        return response;
    }
