...
public class VoucherStatusScheduler {
    ...
    public void updateExpiredVouchers() {
        ...
        try {
            ...
            log.info("Voucher expiry update completed - Expired Vouchers: {}, Expired VoucherUsers: {}",
    ...
    private int updateExpiredVouchers(LocalDateTime now) {
        List<Voucher> expiredVouchers = voucherRepository.findActiveVouchersBeforeExpiryDate(
        ...
        expiredVouchers.forEach(voucher -> {
            ...
            log.debug("Voucher {} ({}) changed to EXPIRED. Expiry date: {}",
                    voucher.getCode(),
                    voucher.getName(),
    ...
    private int updateExpiredVoucherUsers() {
        ...
        expiredVoucherUsers.forEach(voucherUser -> {
            voucherUser.setStatus(VoucherUserStatus.EXPIRED);
            log.debug("VoucherUser ID {} for user {} changed to EXPIRED (Voucher: {})",
                    voucherUser.getId(),
    ...
    public void deactivateOutOfStockVouchers() {
        ...
        try {
            List<Voucher> activeVouchers = voucherRepository.findByStatusAndIsActive(
                    VoucherStatus.ACTIVE,
                    true
            );
            ...
            for (Voucher voucher : activeVouchers) {
                if (voucher.getAvailableQuantity() <= 0) {
                    ...
                    deactivatedCount++;
                    log.debug("Voucher {} changed to OUT_OF_STOCK (no quantity available)",
                            voucher.getCode());
                }
            }
            ...
            log.info("Out-of-stock voucher deactivation completed - {} vouchers deactivated",
                    deactivatedCount);
