...
public class EventStatusScheduler {
    ...
    public void updateEventStatuses() {
        ...
        try {
            ...
            log.info("Event status update completed - Closed: {}, Ongoing: {}, Upcoming: {}",
                    closedCount, ongoingCount, upcomingCount);
        } catch (Exception e) {
    ...
    private int updateClosedEvents(LocalDateTime now) {
        List<Event> eventsToClose = eventRepository.findByStatusAndIsActiveAndEndTimeBefore(
                EventStatus.PUBLISHED,
                true,
                now
        );

        eventsToClose.addAll(eventRepository.findByStatusAndIsActiveAndEndTimeBefore(
                EventStatus.ONGOING,
                true,
                now
        ));
    ...
    private int updateOngoingEvents(LocalDateTime now) {
        List<Event> eventsToStart = eventRepository.findByStatusAndIsActiveAndStartTimeBeforeAndEndTimeAfter(
                EventStatus.PUBLISHED,
                true,
                now,
                now
        ...
        eventsToStart.addAll(eventRepository.findByStatusAndIsActiveAndStartTimeBeforeAndEndTimeAfter(
                EventStatus.UPCOMING,
                true,
                now,
                now
    ...
    private int updateUpcomingEvents(LocalDateTime now, LocalDateTime twoDaysFromNow) {
        List<Event> eventsUpcoming = eventRepository.findByStatusAndIsActiveAndStartTimeBetween(
                EventStatus.PUBLISHED,
                true,
                now,
                twoDaysFromNow
