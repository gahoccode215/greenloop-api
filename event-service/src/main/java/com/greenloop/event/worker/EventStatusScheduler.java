...
public class EventStatusScheduler {
  ...
  private void sendNotifications(Event event, EventStatus status) {
    ...
    userIds.addAll(event.getRegistrations().stream().map(EventRegistration::getUserId).toList());

    for (Long userId : userIds) {

      NotificationEvent.NotificationEventBuilder builder =
          NotificationEvent.builder().userId(userId);
