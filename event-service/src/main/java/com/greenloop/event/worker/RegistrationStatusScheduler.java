    ...
    try {
      List<EventRegistration> bookedRegistrations =
          eventRegistrationRepository.findBookedRegistrationsWithClosedEvents(
              RegistrationStatus.BOOKED, EventStatus.CLOSED, true);
      ...
      for (EventRegistration registration : bookedRegistrations) {
        ...
        log.debug(
            "Registration ID {} changed to NO_SHOW for event: {}",
            registration.getId(),
            registration.getEvent().getCode());
      }

      log.info(
          "Registration status update completed - Updated {} registrations to NO_SHOW",
          updatedCount);
