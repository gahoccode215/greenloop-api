...
public class BlogController {
    ...
    @Operation(summary = "Create new blog", description = "Admin, Manager and Staff can create new blog with optional thumbnail")
    ...
    @PreAuthorize("hasAnyRole('ADMIN', 'MANAGER', 'STAFF')")
    @Operation(summary = "Update blog", description = "Admin, Manager and Staff can update blogs (Staff: own only, Manager/Admin: all)")
    public ResponseEntity<ApiResponseDTO<BlogResponse>> updateBlog(
        ...
        return ResponseEntity.ok(ApiResponseDTO.success("Cập nhật blog thành công", response, HttpStatus.OK));
    }

    @DeleteMapping("/{id}")
    @PreAuthorize("hasAnyRole('ADMIN', 'MANAGER', 'STAFF')")
    @Operation(summary = "Delete blog", description = "Admin and Manager can delete all blogs, Staff can delete own blogs")
    ...
    public ResponseEntity<ApiResponseDTO<BlogResponse>> publishBlog(@PathVariable Long id) {
        ...
        return ResponseEntity.ok(ApiResponseDTO.success("Công khai blog thành công", response, HttpStatus.OK));
    }
    ...
    @GetMapping("/{id}")
    @Operation(summary = "Get blog detail", description = "Get blog detail by ID. Customer can only view published blogs, Admin/Manager/Staff can view all blogs")
    public ResponseEntity<ApiResponseDTO<BlogResponse>> getBlogDetail(@PathVariable Long id) {
        ...
        return ResponseEntity.ok(ApiResponseDTO.success("Lấy thông tin blog thành công", response, HttpStatus.OK));
    }

    @GetMapping("/published")
    @Operation(summary = "Get published blogs", description = "Get list of published blogs with search and pagination. Public access.")
    public ResponseEntity<ApiResponseDTO<PageResponseDTO<BlogResponse>>> getPublishedBlogs(
            @RequestParam(defaultValue = "0") int page,
            @RequestParam(defaultValue = "10") int size,
            @Parameter(description = "Search by title or content") @RequestParam(required = false) String search,
        ...
        return ResponseEntity.ok(ApiResponseDTO.success("Lấy danh sách blog thành công", response, HttpStatus.OK));
    }

    @GetMapping
    @PreAuthorize("hasAnyRole('ADMIN', 'MANAGER', 'STAFF')")
    @Operation(summary = "Get all blogs", description = "Admin, Manager and Staff can view all blogs with search, filter by status and pagination")
    public ResponseEntity<ApiResponseDTO<PageResponseDTO<BlogResponse>>> getAllBlogs(
            @RequestParam(defaultValue = "0") int page,
            @RequestParam(defaultValue = "10") int size,
            @Parameter(description = "Search by title or content") @RequestParam(required = false) String search,
            @Parameter(description = "Filter by status (DRAFT/PUBLISHED/HIDDEN)") @RequestParam(required = false) String status,
    ...
    @GetMapping("/my-blogs")
    @PreAuthorize("hasAnyRole('ADMIN', 'MANAGER', 'STAFF')")
    @Operation(summary = "Get my blogs", description = "Admin, Manager and Staff can view their own blogs with search and pagination")
    public ResponseEntity<ApiResponseDTO<PageResponseDTO<BlogResponse>>> getMyBlogs(
        ...
        return ResponseEntity.ok(ApiResponseDTO.success("Lấy danh sách blog của tôi thành công", response, HttpStatus.OK));
    }
}
