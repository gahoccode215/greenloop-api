...
public class BlogServiceImpl implements BlogService {
  ...
  public BlogResponse createBlog(BlogCreateRequest request, MultipartFile thumbnail) {
    ...
    boolean isAuthorized =
        currentUser.getRoles().stream()
            .anyMatch(
                role ->
                    role.getName().equals(RoleConstants.ADMIN)
    ...
    if (!isAuthorized) {
      throw new UnauthorizedBlogAccessException(
          "Chỉ Admin, Manager hoặc Staff mới có thể tạo blog");
    }
  ...
  public BlogResponse getBlogDetail(Long id) {
    ...
    boolean isStaffOrManagerOrAdmin =
        currentUser.getRoles().stream()
            .anyMatch(
                role ->
                    role.getName().equals(RoleConstants.ADMIN)
  ...
  @Override
  @Transactional(readOnly = true)
  public PageResponseDTO<BlogResponse> getAllBlogs(
